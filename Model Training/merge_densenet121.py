...
def train(model, train_loader, test_loader, val_loader, criterion, optimizer, device):
    ...
    for epoch in range(num_epochs):
        ...
        for ((inputs1, labels1), (inputs2, labels2), _) in train_loader:
            ...
            optimizer.step()  # Update model parameters according to the gradient
            optimizer.zero_grad()  # Clear the gradient and prepare to process the next batch of data
            _, predicted = torch.max(outputs.data,
                                     1)  # Calculate the training accuracy based on the predicted results and the true label
            train_acc += (predicted == labels1).sum().item()
            train_samples += inputs1.size(0)  # Record the number of samples processed
            scheduler.step()  # Update the state of the learning rate scheduler and perform operations such as learning rate decay
        ...
        if val_loss < best_loss:
            best_loss = val_loss
            torch.save(model.state_dict(), '/data/zhenyuan/save_model(outputs)/alldata_random_densenet121_merged_best_model.pth')
            epochs_no_improve = 0
        else:
            ...
            if epochs_no_improve >= patience:
                ...
                # Load the best model
                model.load_state_dict(torch.load('/data/zhenyuan/save_model(outputs)/alldata_random_densenet121_merged_best_model.pth'))
                save_predictions(model, train_loader,
                                 '/home/zhenyuan/mice/results/alldata_densenet121_merged_train_predictions.csv', device)
                # Save the validation set sample name and predicted probability
                save_predictions(model, val_loader,
                                 '/home/zhenyuan/mice/results/alldata_densenet121_merged_val_predictions.csv', device)
                # Save the test set sample name and predicted probability
                save_predictions(model, test_loader,
                                 '/home/zhenyuan/mice/results/alldata_densenet121_merged_test_predictions.csv', device)
                break
...
def main():
    ...
    # Create two independent DenseNet121 models
    model1 = torchvision.models.densenet121(pretrained=False)
    model2 = torchvision.models.densenet121(pretrained=False)
    ...
    weight_decay = 0.1  # Regularization coefficient, which can be adjusted as needed
    ...
    save_predictions(model, train_loader, '/home/zhenyuan/mice/results/alldata_densenet121_merged_train_predictions.csv', device)
    # Save the validation set sample name and predicted probability
    save_predictions(model, val_loader, '/home/zhenyuan/mice/results/alldata_densenet121_merged_val_predictions.csv', device)
    # Save the test set sample name and predicted probability
    save_predictions(model, test_loader,'/home/zhenyuan/mice/results/alldata_densenet121_merged_test_predictions.csv', device)
