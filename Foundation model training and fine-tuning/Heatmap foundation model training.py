...
def save_detailed_progress(info, file_path="/data/zhenyuan/pretrain/save_result/detailed_training_progress.txt"):
    """
...
def save_epoch_info(epoch, timestamp):
    ...
    """
    with open("/data/zhenyuan/pretrain/save_result/epoch_info.txt", "a") as f:
...
def load_previous_state(model, optimizer):
    ...
    """
    if os.path.exists("/data/zhenyuan/pretrain/save_result/model_checkpoint_26.pth"):
        checkpoint = torch.load("/data/zhenyuan/pretrain/save_result/model_checkpoint_26.pth")
...
def save_encoder_parameters(model, save_path):
    # Get all the parameters of the encoder
    encoder_state_dict = model.state_dict()

    # Filter based on the prefix of the parameter name in the encoder
...
def save_checkpoint(epoch, model, optimizer, loss, last_epoch):
    ...
    """
    last_encoder_checkpoint_filename = f"/data/zhenyuan/pretrain/save_result/encoder_checkpoint_{last_epoch}.pth"
    last_checkpoint_filename = f"/data/zhenyuan/pretrain/save_result/model_checkpoint_{last_epoch}.pth"
    ...
    torch.save({
        ...
        'loss': loss
    }, f"/data/zhenyuan/pretrain/save_result/model_checkpoint_{epoch}.pth")



def adjust_learning_rate(optimizer, epoch, warmup_epochs=3, initial_lr=0.01):
...
dataset = CustomImageDataset("/data/zhenyuan/pretrain/heatmap/", transform=transform)
dataloader = DataLoader(dataset, batch_size=64, shuffle=True)

model = mae_vit_large_patch16().cuda(1)
...
for epoch in range(start_epoch, num_epochs):
    ...
    if (epoch + 1) % 1 == 0:
        ...
        last_epoch = epoch + 1  # Update last_epoch to the currently saved epoch
        save_encoder_parameters(model, f'/data/zhenyuan/pretrain/save_result/encoder_checkpoint_{epoch}.pth')

model_save_path = '/data/zhenyuan/pretrain/save_result/vit_pretrain_HM.pth'
