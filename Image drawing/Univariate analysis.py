...
# Read data
train = pd.read_csv(r"E:\Parkinson's disease topic\clinical\clinical data\data set division\first time\train.csv", encoding='ISO-8859-1')
...
# Create a dictionary to map original feature names to new display names
feature_display_names = {
    'Distance': 'Total Movement Distance',
    'Mean speed': 'Average Speed',
    'Line crossings': 'Frequency of Grid Crossings',
    'Center entries': 'Center Zones Entries',
    'Center time': 'Time Spent in Center Zones',
    'Center distance': 'Distance Moved in Center Zones',
    'Around entries': 'Peripheral Zones Entries',
    'Around time': 'Time Spent in Peripheral Zones',
    'Around distance': 'Distance Moved in Peripheral Zones'
}
...
# Create a larger graphic and increase the DPI for better quality
...
for idx, feature in enumerate(existing_features):
    ...
    # Plot the ROC curve using the new display name
    display_name = feature_display_names.get(feature, feature)  # Get the new display name, if not, use the original name
    plt.plot(fpr, tpr, color=colors[idx], lw=2,
             label=f'{display_name} (AUC = {roc_auc:.3f}, 95% CI = [{lower_bound:.3f}, {upper_bound:.3f}])')

# Draw diagonal lines
plt.plot([0, 1], [0, 1], 'k--', lw=1.5, alpha=0.7)

# Set title and labels
plt.title('Discriminative Performance of Behavioral Features', fontsize=16, fontweight='bold')
...
# Set the aspect ratio of the graphic to 1:1
plt.gca().set_aspect('equal', adjustable='box')

# Adjust the layout and save the graph
plt.tight_layout()
plt.savefig('E:\Parkinsons Project\Results\D1.pdf', format='pdf', dpi=300, bbox_inches='tight')

# Display the graph
plt.show()

# Print AUC values and confidence intervals, using new display names
for feature in existing_features:
    fpr, tpr, _ = roc_curve(train['label'], train[feature])
    roc_auc = auc(fpr, tpr)
    ...
    display_name = feature_display_names.get(feature, feature)  # Get the new display name
    print(f"{display_name}: AUC = {roc_auc:.3f}, 95% CI = [{lower_bound:.3f}, {upper_bound:.3f}]")