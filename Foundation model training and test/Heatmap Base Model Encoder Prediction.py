...
def train(model, train_loader, val_loader, optimizer, scheduler, criterion, patience=200, epochs=200):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(model.state_dict(), "/data/zhenyuan/save_model(outputs)/big_model_HM.pth")
            epochs_no_improve = 0
        else:
            epochs_no_improve += 1
            if epochs_no_improve >= patience:
                print(f'Stop training at epoch {epoch} due to early stopping')
                model.load_state_dict(torch.load("/data/zhenyuan/save_model(outputs)/big_model_HM.pth"))
                break
...
train_csv_path = save_predictions(train_data, model, '/home/zhenyuan/mice/results/big_model_HM_train_predictions.csv')
test_csv_path = save_predictions(test_data, model, '/home/zhenyuan/mice/results/big_model_HM_test_predictions.csv')
vali_csv_path = save_predictions(vali_data, model, '/home/zhenyuan/mice/results/big_model_HM_vali_predictions.csv')
