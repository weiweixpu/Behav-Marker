...
data_folder = r'G:\Trajectory Dataset\T-drive Taxi Trajectories\T-drive Taxi Trajectories\taxi_log_2008_by_id'
tracking_folder = r'G:\Trajectory Dataset\T-drive Taxi Trajectories\Trajectory'
heatmap_folder = r'G:\Trajectory Dataset\T-drive Taxi Trajectories\Heatmap'

# Create a folder to store images
os.makedirs(tracking_folder, exist_ok=True)
os.makedirs(heatmap_folder, exist_ok=True)

# Define a function to create a color map
def create_colormap():
    colors = [
...
for file_name in txt_files:
    vehicle_id = file_name[:-4] # Assume the file name prefix is ​​the vehicle ID
    lons, lats = [], []
    ...
    ax.plot(lons, lats, marker='', linestyle='-', color='#854085')
    plt.axis('equal')
    plt.axis('off')
    tracking_filename = os.path.join(tracking_folder, f'{vehicle_id}.png')
    plt.savefig(tracking_filename, bbox_inches='tight', pad_inches=0, facecolor='white', edgecolor='none')
    plt.close(fig)

    # Create colormap and heatmap
    cmap = create_colormap()
    fig, ax = plt.subplots(figsize=(6, 6))
    hist, xedges, yedges = np.histogram2d(lons, lats, bins=[100, 100])
    ...
    plt.axis('off')
    heatmap_filename = os.path.join(heatmap_folder, f'{vehicle_id}.png')
    plt.savefig(heatmap_filename, bbox_inches='tight', pad_inches=0, facecolor='white', edgecolor='none')
    plt.close(fig)

    print(f'Saved tracking and heatmap images for vehicle {vehicle_id}')