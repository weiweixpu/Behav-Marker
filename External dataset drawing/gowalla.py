...
# Parse the data and get the trajectory function of the specified ID
def get_trajectory(data, target_id):
    coordinates = []
    for line in data:
        parts = line.strip().split('\t')
        if parts[0] == str(target_id):
            coordinates.append((float(parts[3]), float(parts[2])))  # (longitude, latitude)
    return coordinates

# Draw trajectory graph function
def plot_trajectory(coordinates, target_id, save_folder):
    if coordinates:
        x_vals, y_vals = zip(*coordinates)

        plt.figure(facecolor='white',figsize=(8, 8))
        plt.plot(x_vals, y_vals, color='#854085', linewidth=2)
        plt.axis('equal')
        plt.axis('off')

        save_path = os.path.join(save_folder, f'track\{target_id}.png')
        plt.savefig(save_path, bbox_inches='tight', pad_inches=0.1)
        plt.close()
    else:
        print("No coordinates found for specified ID.")

# Generate heat map function
def generate_heatmap(coordinates, target_id, save_folder):
    if coordinates:
        x_vals, y_vals = zip(*coordinates)
        hist, x_edges, y_edges = np.histogram2d(x_vals, y_vals, bins=[100, 100])
        ...
        plt.figure(facecolor='white',figsize=(8, 8))
        plt.imshow(hist_smoothed.T, origin='lower', cmap=cmap, extent=[x_edges[0], x_edges[-1], y_edges[0], y_edges[-1]], aspect='auto')
        plt.axis('off')

        save_path = os.path.join(save_folder, f'heatmap\{target_id}.png')
        plt.savefig(save_path, bbox_inches='tight', pad_inches=0)
        plt.close()
    else:
        print("No coordinates found for specified ID.")

# Read the file and return the coordinates organized by ID
def read_data(filename):
    trajectories = {}
    with open(filename, 'r') as file:
        for line in file:
            parts = line.strip().split('\t')
            if len(parts) < 4:  # Skip if it is not a valid line
                continue
            id, _, lat, lon = parts[0], parts[1], float(parts[2]), float(parts[3])
            if id not in trajectories:
                trajectories[id] = []
            trajectories[id].append((lon, lat))  # Longitude first, latitude second
    return trajectories

# File path
filepath = "G:\\Trajectory dataset\\gowalla\\Gowalla_totalCheckins.txt"
save_folder = "G:\\Trajectory dataset\\gowalla"

trajectories = read_data(filepath)

# Traverse each ID and generate a trajectory map and corresponding heat map
for target_id, coordinates in trajectories.items():
    # plot_trajectory(coordinates, target_id, save_folder)
    generate_heatmap(coordinates, target_id, save_folder)
    print(f"Trajectory and heatmap images for ID {target_id} saved.")